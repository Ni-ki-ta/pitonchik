...
class PeopleService:
    ...
    async def get_all(self):
        query = select(PeopleOrm).options(selectinload(PeopleOrm.pets))
    ...
    async def get_person(self, person_id: int) -> PeopleOrm | None:
        ...
        #     options(selectinload(PeopleOrm.pets))
        # )
        # return result.scalars().first()
        result = await self.session.get(PeopleOrm, person_id, options=[selectinload(PeopleOrm.pets)])
        return result
    ...
    async def create(self, data: PeopleAddSchema) -> PeopleOrm:
        ...
        # )
        new_person = PeopleOrm(**data.model_dump())
